...
class TestPlushes(TestCase):
    ...
    def test_plushes_first_page_pagination(self):
        ...
        group = Group.objects.create(name="small",
                                     category=category, slug="small")
        for i in range(10):
            Product.objects.create(name=f"Product {i}", price=10.00,
                                   group=group, slug=f"procuct{i}")
        url = reverse('products:allproducts')
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['products_page']), 8)
...
class TestProductDetail(TestCase):
    ...
    def setUp(self):
        ...
        self.product = Product.objects.create(name="product",
                                              slug='product',
                                              price=10.00, group=group,
                                              code="TESTCODE")
        self.url = reverse('products:productdetail',
                           kwargs={'slug': self.product.slug})

    def test_product_detail_success(self):
        """Tests product_detail renders when a GET request is passed.
            Checks rating_average is calculeted."""
    ...
    def test_product_detail_404(self):
        ...
        invalid_url = reverse('products:productdetail',
                              kwargs={'slug': 'no-slug'})
...
class TestRateProduct(TestCase):
    def setUp(self):
        ...
        self.client = Client()
        category = Category.objects.create(name="Things", slug="3d-prints")
        group = Group.objects.create(name="small", category=category,
                                     slug="small")
        self.product = Product.objects.create(name="product",
                                              slug='product',
                                              price=10.00,
        ...
        self.user = User.objects.create_user(username='testuser',
                                             password='password')
        self.url = reverse('products:rateproduct',
                           kwargs={'slug': self.product.slug})
...
class TestUpdateReview(TestCase):
    def setUp(self):
        ...
        self.owner = User.objects.create_user(username="owner",
                                              password="password2")
        self.non_owner = User.objects.create_user(username="non_user",
                                                  password='password1')
        self.rating = Rating.objects.create(product=self.product,
                                            user=self.owner,
                                            rating='3',
                                            title='ok',
                                            comment='Its ok')
        self.url = reverse('products:updatereview',
                           kwargs={'id': self.rating.id})
